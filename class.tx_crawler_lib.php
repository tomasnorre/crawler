...
class tx_crawler_lib {
	...
	function fe_eofe(&$params, $ref)	{
		if ($params['pObj']->applicationData['tx_crawler']['running'])	{
			...
			echo serialize($params['pObj']->applicationData['tx_crawler']);
		...
		if ($cliObj->cli_argValue('-o')==='url')	{
			...
			foreach($this->queueEntries as $queueRec)	{
				...
				if (is_array($requestResult))	{
					...
					$cliObj->cli_echo('Error: '.substr(ereg_replace('[[:space:]]+',' ',strip_tags($result['content'])),0,10000).'...'.chr(10));
				}
			}
	...
	function CLI_run()	{
		...
		foreach($rows as $r)	{
			$this->readUrl($r['qid']);
			$counter++;
			sleep(1);	// Just to relax the system for a few seconds...(should be configurable?)
