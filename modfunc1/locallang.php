<?php
/**
 * Language labels for module 'crawler'
 *
 * This file is detected by the translation tool.
 */

$LOCAL_LANG = Array (
	'default' => Array (
		'title' => 'Site Crawler',
		'labels.start' => 'Start Crawling',
		'labels.log' =>	'Crawler log',
		'labels.multiprocess' =>	'Crawling Processes',
		'labels.all' =>	'All',
		'labels.pending' =>	'Pending',
		'labels.finished' =>	'Finished',
		'labels.noPageSelected' =>	'Please select a page in the pagetree',
		'labels.noConfigSelected' =>	'Please select at least one configuration',
		'labels.configuration' =>	'Crawl configuration',
		'labels.triggerUpdate' =>	'Update',
		'labels.triggerCrawl' =>	'Crawl URLs',
		'labels.triggerDownload' =>	'Download URLs',
		'labels.continue' =>	'Continue',
		'labels.continueinlog' =>	'Continue and show Log',
		'labels.count' =>	'Count',
		'labels.time' =>	'Time',
		'labels.curtime' =>	'Current server time',
		'labels.submitted' =>	'URLs submitted',
		'labels.depth' =>	'Depth',
		'labels.configurations' =>	'Configurations',
		'labels.scheduled' =>	'Scheduled',
		'labels.pagetitle' =>	'Page title',
		'labels.key' =>	'Key',
		'labels.parametercfg' =>	'Parameter Cfg',
		'labels.values' =>	'Values Expanded',
		'labels.url' =>	'URL',
		'labels.urls' =>	'URLs',
		'labels.options' =>	'Options',
		'labels.parameters' =>	'Parameters',
		'labels.back' =>	'Back',
		'labels.reloadlist' =>	'Reload list',
		'labels.downloadcsv' =>	'Download entries as CSV',
		'labels.flushvisiblequeue' =>	'Flush visible entries',
		'labels.flushfullqueue' =>	'Flush entire queue',
		'labels.confirmyouresure' =>	'Are you sure?',
		'labels.setid' =>	'Set ID',
		'labels.qid' =>	'Queue id',
		'labels.resultlog' =>	'Result Log',
		'labels.scheduledtime' =>	'Scheduled',
		'labels.runtime' =>	'Run-time',
		'labels.status' =>	'Status',
		'labels.groups' =>	'Groups',
		'labels.procinstr' =>	'Proc. Instr.',
		'labels.newprocess' =>	'New process has been started, refresh to monitor the state',
		'labels.newprocesserror' =>	'Error while starting process',
		'labels.time.now' =>	'Now',
		'labels.time.midnight' =>	'Midnight',
		'labels.time.4am' =>	'4 AM',
		'labels.display' =>	'Display',
		'labels.showresultlog' =>	'Show Result Log',
		'labels.showfevars' =>	'Show FE Vars',
		'labels.noentries' =>	'No entries',
		'labels.immediate' =>	'immediate',
		'labels.generalinformation' =>	'General Information',
		'labels.pendingoverview' =>	'Pending Entries (assigned / overall)',
		'labels.processcount' =>	'Processes count (running / max)',
		'labels.clipath' =>	'CLI-Path',
		'labels.processstates' =>	'Process States',
		'labels.state' =>	'State',
		'labels.processid' =>	'Id',
		'labels.time.first' =>	'Time first item',
		'labels.time.last' =>	'Time last item',
		'labels.time.duration' =>	'Runtime',
		'labels.ttl' =>	'TTL',
		'labels.status.current' =>	'Processed',
		'labels.status.initial' =>	'Initially assigned',
		'labels.status.finally' =>	'Finally assigned',
		'labels.status.progress' =>	'Progress',
		'labels.process.running' =>	'Process running',
		'labels.process.success' =>	'Process completed successfully',
		'labels.process.chanceled' =>	'Process was cancelled',
		'labels.refresh' =>	'Refresh',
		'labels.disablecrawling' =>	'Stop all processes and disable crawling',
		'labels.enablecrawling' =>	'Enable crawling',
		'labels.show.running' =>	'Show only running processes',
		'labels.show.all' =>	'Show finished and terminated processes',
		'labels.process.add' =>	'Add process',
		'time.detailed' =>	'd-m-y H:i:s',
	),
	'dk' => Array (
	),
	'de' => Array (
	),
	'no' => Array (
	),
	'it' => Array (
	),
	'fr' => Array (
	),
	'es' => Array (
	),
	'nl' => Array (
	),
	'cz' => Array (
	),
	'pl' => Array (
	),
	'si' => Array (
	),
	'fi' => Array (
	),
	'tr' => Array (
	),
	'se' => Array (
	),
	'pt' => Array (
	),
	'ru' => Array (
	),
	'ro' => Array (
	),
	'ch' => Array (
	),
	'sk' => Array (
	),
	'lt' => Array (
	),
	'is' => Array (
	),
	'hr' => Array (
	),
	'hu' => Array (
	),
	'gl' => Array (
	),
	'th' => Array (
	),
	'gr' => Array (
	),
	'hk' => Array (
	),
	'eu' => Array (
	),
	'bg' => Array (
	),
	'br' => Array (
	),
	'et' => Array (
	),
	'ar' => Array (
	),
	'he' => Array (
	),
	'ua' => Array (
	),
	'lv' => Array (
	),
	'jp' => Array (
	),
	'vn' => Array (
	),
	'ca' => Array (
	),
	'ba' => Array (
	),
	'kr' => Array (
	),
);
?>